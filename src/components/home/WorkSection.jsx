...
const categoryLogos = {
  ...
  'packaging-print': (
    <svg className="w-4 h-4 md:w-5 md:h-5" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24">
      <path d="M21 8l-9-4-9 4m18 0l-9 4m9-4v8l-9 4m0-12L3 8m9 4v8m-9-12v8l9 4" />
    </svg>
  ),
  'visual': (
    <svg className="w-4 h-4 md:w-5 md:h-5" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24">
  ...
  '3d': (
    <svg className="w-4 h-4 md:w-5 md:h-5" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24">
      <path d="M12 2L3 7v10l9 5 9-5V7l-9-5z" /><path d="M12 22V12" /><path d="M21 7l-9 5L3 7" />
    </svg>
  ),
  'motion': (
    <svg className="w-4 h-4 md:w-5 md:h-5" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24">
  ...
  'experimental': (
    <svg className="w-4 h-4 md:w-5 md:h-5" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24">
      <path d="M9 3h6M10 3v7l-4 8a2 2 0 002 2h8a2 2 0 002-2l-4-8V3" />
    </svg>
  )
};
...
const ProjectCard = memo(({ item, onMouseEnter, onMouseLeave, onSelect }) => {
  ...
  return (
    <div 
      ...
      onMouseEnter={() => onMouseEnter(item.bgColor || '#1d1d1d')} 
      onMouseLeave={onMouseLeave}
      onClick={() => !isProject && onSelect(item)}
    >
      {isProject ? (
        <Link to={`/project/${item.id}`} className="block text-white transition-all duration-500">
          <div className="rounded-xl overflow-hidden mb-6 bg-zinc-900 h-[350px] md:h-[550px] w-auto inline-block relative">
            <img 
              ...
              className="h-full w-auto object-contain group-hover/card:scale-105 transition-all duration-1000" 
              alt={item.client} 
            />
          </div>
          <div className="flex justify-between items-start px-1">
            <div className="text-left">
              <h3 className="font-bold text-xl md:text-2xl uppercase tracking-tighter leading-none mb-1">{item.client}</h3>
              <p className="text-[9px] md:text-[10px] text-white/40 uppercase tracking-widest font-medium">{item.project}</p>
            </div>
            <div className="text-right">
              <span className="text-[9px] md:text-[10px] text-[#c792ff] font-black uppercase tracking-[0.2em]">
        ...
        <div className="h-[400px] md:h-[650px] w-auto rounded-xl overflow-hidden bg-zinc-900 border border-white/5 hover:border-white/20 transition-colors">
           {isVideo 
             ? <video src={item.src} autoPlay muted loop playsInline className="h-full w-auto object-contain" /> 
             : <img src={item.src} className="h-full w-auto object-contain" alt="Asset" />
           }
        </div>
      )}
    </div>
...
const WorkSection = () => {
  ...
  // LOGIC: Hide Progress Bar by adding class to body
  useEffect(() => {
    if (selectedAsset) {
  ...
  const dynamicDuration = useMemo(() => {
    return filteredItems.length * SECONDS_PER_ITEM;
  }, [filteredItems]);
      ...
      <AnimatePresence>
        {selectedAsset && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center">
            {/* Backdrop with blur - separated from content */}
            <motion.div 
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
            ...
            {/* Close Button - Outside the blurred container */}
            <motion.button 
              initial={{ opacity: 0, y: -20 }}
              animate={{ opacity: 1, y: 0 }}
              exit={{ opacity: 0, y: -20 }}
              className="absolute top-10 right-10 w-14 h-14 rounded-full bg-white/10 hover:bg-white/20 border border-white/20 flex items-center justify-center text-white transition-all z-[120] active:scale-90"
              onClick={() => setSelectedAsset(null)}
            >
            ...
            <motion.div 
              ...
              className="relative w-full h-full flex items-center justify-center p-4 md:p-12 z-[110] pointer-events-none"
            >
        ...
        <div className="flex justify-center w-full">
          <div className="flex flex-wrap justify-center gap-4 md:gap-6">
            {[
              ...
              <button 
                key={f.id} 
                onClick={() => { setActiveFilter(f.id); setIsPaused(false); }} 
                className={`group relative flex items-center gap-3 px-6 py-3 md:px-10 md:py-4 rounded-full transition-all duration-500 text-[10px] md:text-xs font-black uppercase tracking-widest cursor-pointer backdrop-blur-2xl ${
                  activeFilter === f.id 
                  ? 'bg-black/90 text-white ring-1 ring-white/40 scale-105 shadow-[0_0_30px_-5px_rgba(199,146,255,0.4)]' 
                  : 'bg-white/10 border border-white/20 text-white hover:border-white/40 hover:bg-white/20'
                }`}
              >
                {activeFilter === f.id && (
                  <div className="absolute inset-0 rounded-full opacity-30 blur-xl -z-10 bg-gradient-to-r from-pink-500 via-purple-500 to-blue-500 animate-pulse" />
                )}
                <span>{categoryLogos[f.id]}</span>
            ...
            >
              {filteredItems.map((item, idx) => (
                <ProjectCard 
                  key={idx} 
                  item={item} 
                  onMouseEnter={(color) => { document.body.style.backgroundColor = color; }} 
