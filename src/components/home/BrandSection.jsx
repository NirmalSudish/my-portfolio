  ...
  return (
    <div className="text-center px-6 w-full max-w-[90vw] lg:max-w-7xl mx-auto flex flex-col items-center justify-center">
      ...
      <motion.div 
        ...
        className="text-2xl md:text-5xl lg:text-6xl font-medium text-white/90 leading-[1.2] mb-20 uppercase tracking-tight"
      >
        ...
        <div className="md:whitespace-nowrap">I BUILD <span className="text-white font-black">HIGH-END DIGITAL PRODUCTS</span></div>
        <div className="md:whitespace-nowrap">THAT BRIDGE THE GAP BETWEEN COMPLEX</div>
        <div className="md:whitespace-nowrap">ENGINEERING AND <span className="text-[#c792ff] font-black italic">HUMAN-CENTERED UI/UX DESIGN.</span></div>
      </motion.div>

      {/* CLIENT MARQUEE PILL SECTION */}
      <div className="flex flex-col items-center w-full max-w-4xl">
        {/* Color updated to text-white for high visibility */}
        <p className="text-xs font-black text-white uppercase tracking-[0.5em] mb-10">
          Clients I have worked for
        </p>
        
        <div className="bg-white/[0.03] backdrop-blur-md border border-white/10 rounded-full py-8 px-12 w-full overflow-hidden">
          ...
          <div className="flex animate-marquee-slow w-max gap-20 items-center grayscale opacity-40">
            {logos.map((logo, i) => (
              <div key={i} className="flex flex-col items-center gap-2">
                <img src={logo.src} className="h-5 md:h-7 w-auto brightness-0 invert" alt={logo.name} />
                <span className="text-[8px] tracking-widest text-white uppercase">{logo.name}</span>
              </div>
            ))}
          </div>
        </div>
